...
openssl req -config /etc/ssl/openssl.cnf -new -keyout /manzoloCA/intermediate/private/CA_INTERMEDIATE.key.pem -out /manzoloCA/intermediate/csr/CA_INTERMEDIATE.csr.pem -subj "/C=IT/ST=Toscana/L=Scarperia e San Piero/O=Manzolo Organization/OU=Manzolo Intermediate CA/CN=Manzolo Intermediate Certification Authority"

# Firma la CSR con la CA root
openssl ca -config /manzoloCA/openssl.cnf -extensions v3_intermediate_ca -days 1825 -notext -md sha256 -in /manzoloCA/intermediate/csr/CA_INTERMEDIATE.csr.pem -out /manzoloCA/intermediate/certs/CA_INTERMEDIATE.cert.pem
